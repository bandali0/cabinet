    ...
    public void execute(final OutputStream os, final SftpClient.CompletionCallback callback) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (requiresRoot()) {
                    try {
                        List<String> results = runAsRoot(getPath());
                        StringBuilder str = new StringBuilder();
                        for (String item : results) {
                            str.append(item);
                            str.append("\n");
                        }
                        byte[] toWrite = str.toString().getBytes();
                        os.write(toWrite);
                ...
                try {
                    mExecutor = Runtime.getRuntime().exec(getPath());
                    InputStream is = mExecutor.getInputStream();
                    int numRead;
                    byte[] bytes = new byte[1000];
                    while (true) {
                        numRead = is.read(bytes);
                        if (mExecutor == null) break;
                        else if (numRead == 0) {
                            try {
                                Thread.sleep(500);
